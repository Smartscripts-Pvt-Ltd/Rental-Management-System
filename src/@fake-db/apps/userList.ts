...
mock.onGet('/apps/users/list').reply(config => {
  const { q = '', role = null, status = null, current_plan = null } = config.params ?? ''
    ...
    user =>
      (user.username.toLowerCase().includes(queryLowered) ||
        ...
        (user.email.toLowerCase().includes(queryLowered) &&
          user.current_plan.toLowerCase().includes(queryLowered) &&
          user.status.toLowerCase().includes(queryLowered))) &&
      user.role === (role || user.role) &&
      user.current_plan === (current_plan || user.current_plan) &&
